...
lambda_norm = handles.model.sigma2c ./ sum(handles.model.sigma2c) .* 100;

% Parameter sliders
handles.param = {};
handles.paramslider = {};
for i=1:length(handles.model.sigma2c)
  handles.param{i} = uipanel(...
    'BorderType'   , 'none', ...
  ...
  uicontrol(...
    'Style'        , 'text', ...
    'Units'        , 'pixels', ...
    'String'       , sprintf('%.1f%%', lambda_norm(i)), ...
    'parent'       , handles.param{i}, ...
    'Position'     , [129 0 35 15], ...
...
for i=1:length(handles.paramslider)
  c(i) = get(handles.paramslider{i}, 'Value');
end
...
for i=1:length(handles.paramslider)
  set(handles.paramslider{i}, 'Value', 0);
end
param_Callback(hObject, eventdata, handles);
...
for i=1:length(handles.paramslider)
  set(handles.paramslider{i}, 'Value', c(i));
end
param_Callback(hObject, eventdata, handles);
